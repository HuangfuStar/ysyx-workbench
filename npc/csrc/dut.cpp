...
svScope find_debug_scope() {
  ...
  for (int i = 0; i < ARRLEN(kScopeCandidates); ++i) {
    const char *name = kScopeCandidates[i];
...
const char *sim_get_gpr_name(int idx) {
  ...
  const char *const *names = (gpr_num == MUXDEF(CONFIG_ISA_RISCV32E, 16, 32)) ? kGprNames16 : kGprNames32;
